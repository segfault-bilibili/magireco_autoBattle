...
var scr = {
  res: {width: 0, height: 0}, //当前真实屏幕的分辨率，或者异形屏下，游戏画面（没占满整个屏幕）的分辨率
  ratio: {x: 0, y: 0},        //宽高比
  ref: {                      //ref是假想的16:9参照屏幕，宽或高放缩到和当前屏幕一样
    width: 0, height: 0,
    offset: {
      cutout: {x: 0, y: 0},   //DisplayCutout 异形屏，屏幕有一部分不用来正常显示
...
//初始化坐标换算，填入各种参数，包括屏幕（游戏画面）宽度、高度、异形屏X/Y偏移等
function initCoordsConversion(width, height, cutoutOffsetX, cutoutOffsetY) {
    scr.res.width = width;
    scr.res.height = height;
    scr.ref.offset.cutout.x = cutoutOffsetX;
    scr.ref.offset.cutout.y = cutoutOffsetY;

    let gcd = get_gcd(scr.res.width, scr.res.height);
    //得到宽高比
...
function convertCoords(d) {
    ...
    actual.x += scr.ref.offset.cutout.x; //异形屏，屏幕有一部分不用来正常显示
    actual.y += scr.ref.offset.cutout.y; //同上
