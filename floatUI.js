...
floatUI.scripts = [
    {
        name: "副本周回(剧情/活动通用)",
        fn: tasks.default,
        introduction: "自动周回进行主线/支线/活动剧情副本战斗，会根据设置自动吃AP药。配合“优先使用官方自动续战”可以加快周回速度。\n"
                     +"一般情况下推荐在助战选择界面启动。对于杜鹃花型活动则推荐在活动剧情地图上启动。目前暂不支持在素材目录启动。\n"
                     +"支持闪退/掉线（强制回首页）自动重开，支持无条件定时杀进程重开（用于避免因游戏内存泄漏、内存占用持续爬升而导致系统崩溃的问题），但是要使用这个自动重开功能，必须先录制或导入选关动作录制数据。\n"
                     +"使用闪退自动重开功能时，强烈推荐同时启用“防断线模式”，否则战斗中途如果发生掉线，仍然会卡在询问是否重连的对话框不动。\n"
                     +"即便缺少选关动作录制数据（或者故意不启用闪退重开功能），“防断线模式”在这种情况下仍然可用，但“防断线模式”仅限于在战斗过程中不断点击断线重连按钮所在位置，并不能应对其他时刻出现的掉线问题。\n"
                     +"支持暂停运行（目前只有这一个脚本支持暂停，其他脚本暂不支持暂停），想要暂停时，请点击悬浮窗的灰色齿轮按钮，然后就会打开脚本设置界面，并暂停运行脚本；在脚本设置界面可以修改参数（立即生效），修改完毕后可以点击“返回游戏并继续运行脚本”按钮。\n"
                     +"\n"
                     +"最后提个小小的建议：如果是在“水波祭”期间，目的是获取最多的Pt，本着人人为我我为人人的精神，建议每天先暂时不要开启“优先使用使用官方自动续战”，这样就不会再锁定某一个助战，从而能够照顾到更多互关好友，让好友在第二天登录时也能领到更多Pt。",
    },
    {
        name: "镜层周回",
        fn: tasks.mirrors,
        introduction: "自动周回进行镜层战斗，会根据设置自动吃BP药。\n"
                     +"默认开启“智能挑选最弱对手”，会挨个点开三个匹配到的对手，根据队伍人数和等级情况估算平均战力，从而挑选出平均战力最弱的对手。如果发现总战力低于我方六分之一的队伍则视为福利队直接选择开始战斗。\n"
                     +"不过“智能挑选最弱对手”存在一个已知问题：会概率性地无法获取某个对手的战力。这种情况下多次尝试失败会暂时回退到直接选择第3个对手的模式。\n"
                     +"战斗模式分两种，默认开启识图自动战斗，通过截屏识图来实现更优的选盘，同时支持连携和主动技能（目前只是从第3回合起闭着眼全部放出去；除此之外局限还有：目前选盘时还不能识别Blast方向、暂不支持Magia/Doppel大招）。\n"
                     +"识图自动战斗虽然能够提升胜率，但截屏识图本身可能带来不稳定性，比如截屏权限可能会在一段时间后意外丢失。另外，某些环境下（主要是模拟器）安卓系统的录屏API返回的只有空数据。\n"
                     +"如果碰到上述问题，可尝试开启“使用root或adb权限截屏”来尽量避开问题，但是正如其名，开启它必须有root或（Shizuku）adb授权，而且即便成功开启，截图效率也会明显下降（相比不开启时变慢不少、占用更多CPU时间）。\n"
                     +"（挑选最弱对手本身并不需要识图，只有进入战斗后才会用到识图）\n"
                     +"停用识图自动战斗后，则是在战斗中无脑点击前3个盘所在位置，直到战斗结束。考虑到点击行动盘在某些情况下会偶尔点不到，所以，点击前3个盘时可能会故意暂时跳过某些盘不点，从而抵消掉偶尔点盘没反应的问题，避免在点击行动盘时出现死循环（“镜层周回2(备用,无脑123盘)”脚本没有加入这个逻辑）。",
    },
    {
        name: "自动点击行动盘(识图,连携)",
        fn: tasks.CVAutoBattle,
        introduction: "只能在进入战斗后启动，启动后会自动选盘、打完本局战斗，然后结束。“镜层周回”脚本在默认启用识图自动战斗的情况下，每一局战斗调用的就是这个脚本。\n"
                      +"虽然最常用于镜层，但在剧情副本战斗中也可以启用，可以应对没有初回AUTO（现在很少碰到了）的情况。强烈推荐只在我方战力保证能够碾压剧情副本敌人时使用这个脚本！\n"
                      +"通过截屏识图来实现更优的选盘，同时支持连携和主动技能（目前只是从第3回合起闭着眼全部放出去；除此之外局限还有：目前选盘时还不能识别Blast方向、暂不支持Magia/Doppel大招）。\n"
                      +"识图自动战斗虽然能够提升胜率，但截屏识图本身可能带来不稳定性，比如截屏权限可能会在一段时间后意外丢失。另外，某些环境下（主要是模拟器）安卓系统的录屏API返回的只有空数据。\n"
                      +"如果碰到上述问题，可尝试开启“使用root或adb权限截屏”来尽量避开问题，但是正如其名，开启它必须有root或（Shizuku）adb授权，而且即便成功开启，截图效率也会明显下降（相比不开启时变慢不少、占用更多CPU时间）。",
    },
    {
    ...
    },
    {
        ...
        introduction: "正如其名，这个脚本用来录制一连串的“点击/拖动/检测文字”等动作，用来在闪退/掉线自动重开的时候自动重新选关。\n"
                     +"目前这个脚本只能配合“副本周回(剧情/活动通用)”的闪退自动重开功能使用，但是未来可能会开发出更广泛的用途。\n"
                     +"在选关动作录制完成后，强烈推荐多次测试重放选关动作（很显然，每次重放开始前都需要先回到首页），确保一切无误！",
    },
    {
        name: "重放选关动作",
        fn: tasks.replaySteps,
        introduction: "正如其名，这个脚本用来重放之前录制（或导入）的一连串的“点击/拖动/检测文字”等动作。\n"
                     +"这个脚本的主要用途，是“副本周回(剧情/活动通用)”的脚本在执行闪退/掉线自动重开时，被“副本周回(剧情/活动通用)”自动调用，用来在游戏刚刚重新登录后继续自动完成选关。\n"
                     +"除此之外，在录制（或导入）选关动作录制数据后，一般也需要手动启动这个脚本来进行测试。\n"
                     +"（重放每一步操作时都会进行强制的“安全检查”，如果发现是在“抽卡/记忆结晶强化或界限突破”等界面，则停止重放，尽最大可能避免产生危险的误操作）\n"
                     +"在选关动作录制完成后，强烈推荐多次测试重放选关动作（很显然，每次重放开始前都需要先回到首页），确保一切无误！",
    },
    {
        name: "导入动作录制数据",
        fn: tasks.importSteps,
        introduction: "正如其名，这个脚本用来导入动作录制数据。\n"
                     +"因为不同的屏幕分辨率下，坐标会有区别，强行套用会产生错位，所以目前只支持导入屏幕参数相同的录制数据，除非要导入的数据经过特别设计、可以适配各种屏幕分辨率、把自己标记为“通用”。",
    },
    {
        name: "导出动作录制数据",
        fn: tasks.exportSteps,
        introduction: "正如其名，这个脚本用来导出动作录制数据。",
    },
    {
        name: "清除动作录制数据",
        fn: tasks.clearSteps,
        introduction: "正如其名，这个脚本用来清除动作录制数据。\n"
    ...
    },
    {
        ...
        introduction: "在选择助战时，脚本默认会优先选互关好友（Pt收益最多），如果没有互关好友则选择Pt收益同样是最多的NPC，如果没有NPC则会选择单向好友或路人助战。\n"
                     +"（未来还可能加入优先选高活动加成记忆结晶、龙城明日香等指定角色助战的功能）\n"
                     +"说起来简单，实现起来却很让人头疼，因为不同环境下的情况不一样。这个功能已经被重写过不止一遍了。\n"
                     +"目前，助战自动选择功能为了规避某些环境下读取到的控件坐标数据严重歪曲的问题，是先“数”出各类助战各有多少个，然后“闭着眼”推算出需要点击的坐标。\n"
                     +"所以，在进行助战自动选择的时候，才会提示用户不要拖动助战列表，以防点击错位。\n"
                     +"这个“测试助战自动选择”脚本可以更方便地验证助战自动选择功能是否可以工作正常，一般用于辅助开发者排查问题。",
    },
    {
        name: "测试闪退自动重开",
        fn: tasks.testReLaunch,
        introduction: "部分手机会拦截关联启动，从而导致脚本无法正常完成闪退/掉线自动重开。\n"
                     +"但是一般情况下，用户都可以手动选择“放行”或“允许”，让系统不要拦截脚本。MIUI下，只要选过一次“允许”，这个问题就被排除了，闪退/掉线自动重开就可以正常工作了。\n"
                     +"强烈建议在使用闪退自动重开功能前先多次测试闪退自动重开，确保一切无误！",
    },
    {
        name: "通用副本周回v3.6.0(备用)",
        fn: tasks.default3_6_0,
        introduction: "这个“通用副本周回v3.6.0(备用)”脚本来自3.6.0旧版，这个版本在开发者大致印象中问题比较少。\n"
                     +"除非万不得已，否则不推荐使用这个旧版脚本。",
    },
    {
        name: "副本周回2(备用可选)",
        fn: autoMain,
        introduction: "这个“副本周回2(备用可选)”脚本来自更陈旧的版本。\n"
                     +"当时，群里的一位大佬贡献了重写过的“控件版”脚本，“控件版”就是当前这个版本的基石。\n"
                     +"当时是为了避免代码合并冲突，以及在新的重写过的脚本出问题时还有选择余地，才保留了重写前的旧版脚本。\n"
                     +"除非万不得已，否则不推荐使用这个旧版脚本。",
    },
    {
        name: "活动周回2(备用可选)",
        fn: autoMainver1,
        introduction: "这个“活动周回2(备用可选)”脚本来自更陈旧的版本。\n"
                     +"当时，群里的一位大佬贡献了重写过的“控件版”脚本，“控件版”就是当前这个版本的基石。\n"
                     +"当时是为了避免代码合并冲突，以及在新的重写过的脚本出问题时还有选择余地，才保留了重写前的旧版脚本。\n"
                     +"除非万不得已，否则不推荐使用这个旧版脚本。",
    },
    {
        ...
        fn: jingMain,
        introduction: "这个“镜层周回2(备用,无脑123盘)脚本来自更陈旧的版本。\n"
                     +"当时，群里的一位大佬贡献了重写过的“控件版”脚本，“控件版”就是当前这个版本的基石。\n"
                     +"当时是为了避免代码合并冲突，以及在新的重写过的脚本出问题时还有选择余地，才保留了重写前的旧版脚本。\n"
                     +"（另外，现在的新版“镜层周回”脚本以及识图自动战斗实际上来自于“魔改版”而不是“控件版”，“控件版”并没有涉及镜层这一方面）。\n"
                     +"除非万不得已，否则不推荐使用这个旧版脚本。",
    },
    {
        name: "活动周回,自动重开(备用)",
        fn: tasks.reopen,
        introduction: "这个备用的活动周回脚本,功能是刷门票型(DAILYTOWER)活动(类似[初出茅庐女仆十七夜 阔达自在！]这种)。"
